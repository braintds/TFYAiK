...
namespace Scanner
{
    public static class LexicalScanner
    {
        ...
        public static bool ProbablyTheOperator(string text) 
        {
            if (LevenshteinDistance(text, "and") == 1 )
            {
                return true;
            }
            if (LevenshteinDistance(text,"not") == 1)
            {
                return true;
            }
            if(LevenshteinDistance(text, "xor") == 1)
            {
                return true;
            }
            if (LevenshteinDistance(text, "or") == 1)
            {
                return true;
            }
            if (LevenshteinDistance(text, "div") == 1)
            {
                return true;
            }
            if (LevenshteinDistance(text, "mod") == 1)
            {
                return true;
            }
            return false;
        ...
        public static List<LexicalItem> GetTokens(string inputString)
        {
            ...
            while (i < inputString.Length)
            {
                ...
                if (Char.IsLetter(c))
                {
                    ...
                    if (ProbablyTheOperator(subString)) 
                    {
                        parts.Add(new LexicalItem(Codes.ProbablyOp, subString, start, i));
                        continue;
                    }
